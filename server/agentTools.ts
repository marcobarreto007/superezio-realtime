...
export async function executeTool(toolName: string, parameters: Record<string, any> = {}, confirmed: boolean = false): Promise<ToolExecution> {
  ...
  try {
    ...
    switch (toolName) {
      ...
      case 'search_files':
        // Resolver Desktop do usuário se mencionado
        let searchPath = parameters.searchPath;
        if (searchPath && (searchPath.toLowerCase().includes('desktop') || !path.isAbsolute(searchPath))) {
          const userHome = os.homedir();
          if (searchPath.toLowerCase().includes('desktop')) {
            searchPath = path.join(userHome, 'Desktop', searchPath.replace(/desktop/gi, '').replace(/^[\/\\]+/, ''));
          } else {
            // Caminho relativo - resolver a partir do Desktop por padrão
            searchPath = path.join(userHome, 'Desktop', searchPath);
          }
        }
        const normalizedSearchPath = normalizePath(searchPath);
        result = await searchFilesRecursive(normalizedSearchPath, parameters.pattern);
        break;
